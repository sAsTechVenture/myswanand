...
function DietitianConsultationContent() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      if (searchQuery !== urlSearch) {
        ...
        params.set('page', '1');
        localizedRouter.push(`/dietitian-consultation?${params.toString()}`);
      }
    }, 500);
  ...
  useEffect(() => {
    async function fetchDietitians() {
      try {
        ...
        const url = `/patient/consultants/dietitians${queryString ? `?${queryString}` : ''}`;
        ...
        if (response.data.success && response.data.data) {
          setDietitians(response.data.data.data || []);
          setPagination(response.data.data.pagination);
        }
      } catch (err) {
        console.error('Error fetching dietitians:', err);
        toast.error('Failed to load dietitians. Please try again.');
      } finally {
        setLoading(false);
      }
    }
  ...
  // Fetch available slots only when dietitian is selected
  useEffect(() => {
    ...
    async function fetchSlots() {
      ...
      try {
        ...
        const params = new URLSearchParams({
          consultantId: selectedDietitian.id,
        });
  ...
  const handleBookAppointment = async () => {
    if (!selectedDietitian || !selectedTimeSlot) {
      toast.error(t('common.pleaseSelectDoctorTime'));
      return;
    }

    if (!isAuthenticated()) {
      localizedRouter.push('/auth/login?redirect=/dietitian-consultation');
      return;
    }
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-6">
          ...
          <h1
            ...
            style={{ color: colors.primary }}
          >
            {t('common.dietitianConsultation')}
          </h1>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Left Column - Dietitian List */}
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="p-4">
              <div className="relative">
                ...
                <Input
                  type="text"
                  placeholder={t('common.searchDietitianByName')}
                  value={searchQuery}
            ...
            {loading ? (
              ...
              <Card className="p-12 text-center">
                <p className="text-gray-600">{t('common.noDietitiansFound')}</p>
              </Card>
            ) : (
              <div className="space-y-4">
                {dietitians.map((dietitian) => {
                  const imageUrl = getImageUrl(dietitian.profileImage);
                  const isSelected = selectedDietitian?.id === dietitian.id;
          ...
          <div className="lg:col-span-1">
            <Card className="p-6 sticky top-4">
              ...
              <div className="mb-6">
                ...
                {loadingSlots ? (
                  ...
                  <p className="text-sm text-gray-500 text-center py-4">
                    {!selectedDietitian
                      ? t('common.pleaseSelectDoctorTime')
                      : 'No slots available'}
                  </p>
                ) : (
                  <div className="space-y-4">
              ...
              <Button
                ...
                disabled={!selectedDietitian || !selectedTimeSlot || booking}
                style={{
                  backgroundColor: colors.primary,
                  color: colors.white,
                }}
...
export default function DietitianConsultationPage() {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen bg-gray-50">
